...
public class MetadataController extends HearthstoneController {
    ...
    @GetMapping("/sets")
    public Mono<ApiResponse<List<CardSet>>> getCardSetMetadata(@Valid @ModelAttribute MetadataRequest request) {
        return makeRequest(request, PATH + "/sets", new ParameterizedTypeReference<List<CardSet>>() {})
            .map(ApiResponse::ok);
    }

    @GetMapping("/rarities")
    public Mono<ApiResponse<List<Rarity>>> getRarityMetadata(@Valid @ModelAttribute MetadataRequest request) {
        return makeRequest(request, PATH + "/rarities", new ParameterizedTypeReference<List<Rarity>>() {})
            .map(ApiResponse::ok);
    }

    @GetMapping("/classes")
    public Mono<ApiResponse<List<HeroClass>>> getClassMetadata(@Valid @ModelAttribute MetadataRequest request) {
        return makeRequest(request, PATH + "/classes", new ParameterizedTypeReference<List<HeroClass>>() {})
            .map(ApiResponse::ok);
    }

    @GetMapping("/keywords")
    public Mono<ApiResponse<List<Keyword>>> getKeywordMetadata(@Valid @ModelAttribute MetadataRequest request) {
        return makeRequest(request, PATH + "/keywords", new ParameterizedTypeReference<List<Keyword>>() {})
            .map(ApiResponse::ok);
    }

    @GetMapping("/filterOptions")
    public Mono<ApiResponse<FilterResponse>> getFilterOptions(@Valid @ModelAttribute MetadataRequest request) {
        return metadataService.getFilterOptions(request)
            .map(ApiResponse::ok);
    }
